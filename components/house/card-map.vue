<template>
  <div class="house_col" v-bind:class="[ (is_viewed === '1' ? 'viewed_depricate': ' ') , (is_viewed === 1 ? 'viewed_depricate': ' '),(col === 'col4' ? 'col-4' : 'col') , (col === 'col6' ? 'col-6' : 'col')]">
    <div>
      <div class="row">
        <div class="card pico_card" style="box-shadow: 0 0 10px 0 #0000001a;margin: auto 10px;">

          <div class="card-image" v-if="(typeof images !== 'undefined') && (images.length === 0) " style="height: 180px !important;" :style="{ backgroundImage: 'url(' + require('@/assets/images/sample_building/no-image.jpg') + ')' }" @click="gotopage" ></div>
<!--          <div class="card-image" v-if="(typeof images !== 'undefined') && (images.length > 0) " style="height: 180px !important;" :style="{ backgroundImage: 'url(' + images[0].img + ')' }" @click="gotopage" ></div>-->
          <img v-if="(typeof images !== 'undefined') && (images.length > 0)" v-bind:class="[{'pico_left_card' : radius == 'right' },{'pico_right_card' : radius == 'left'}]" :src="images[0].img" class="card-image" style="object-fit: cover;height: 180px !important;" @click="gotopage" >

          <div v-if="is_special === 1" class="special_badge text-center"> {{$t('house_card.special_adv')}} </div>
          <div v-if="card_is_like === 0" class="like_icon_off text-center" :content="$t('house_card.favorite')" v-tippy="{placement : 'left'}" @click="like_estate"> <i class="far fa-heart like_ico_off"></i> </div>
          <div v-if="card_is_like === 1" class="like_icon text-center" :content="$t('house_card.favorite')" v-tippy="{placement : 'left'}" @click="unlike_estate"> <i class="far fa-heart like_ico"></i> </div>

          <div v-if="project_id != '0'" class="sale_badge text-center" style="margin-bottom: 5px;" > {{$t('house_card.project')}} </div>
          <div v-if="dealing_type == '5'" class="sale_badge text-center" style="margin-bottom: 5px;" @click="gotopage"> {{$t('house_card.sale')}} </div>
          <div v-if="dealing_type == '6'" class="sale_badge text-center" style="margin-bottom: 5px;" @click="gotopage"> {{$t('house_card.mortgage')}} </div>
      ...
      <div class="row">
        <div class="card detail_card" style="margin: auto 10px;">
          <div class="card-body" style="padding: .7rem;padding-top: 0;min-height: 13vh;">

            <h5 class="card-title Estedad_Bold" @click="gotopage"> {{post_title}} </h5>
            <h6 class="card-subtitle mb-2 text-muted Estedad_Light" @click="gotopage"> {{small_address}} </h6>
            <p v-if="price != undefined" class="card-text card-price" style="margin-bottom: auto;" @click="gotopage">{{ price | numeral('0,0') }} {{price_unit}}</p>
            <p v-if="price == undefined" style="font-size: 14px;margin-bottom: auto;" class="card-text card-price" @click="gotopage">{{$t('house_card.adaptive')}}</p>


            <div v-if="is_ifram_360 === 1" class="dark_icon" style="top: 35px;padding-left: 0;" :content="$t('house_card.birds_eye')" v-tippy="{placement : 'right'}" @click="gotopage_bird_view">
            ...
            <div v-if="is_matterport === 1" class="dark_icon" style="top: 70px;padding-left: 0;" :content="$t('house_card.view_interior')" v-tippy="{placement : 'right'}" @click="gotopage_3d">
              <img alt="EVARA"  :src="require(`@/assets/images/evara_icon/3d_2.png`)" style="margin-top: 5px" class="dark_ico">
            </div>
          ...
          <div class="card-footer text-muted">
            ...
            <div class="row mt-1" style="direction: rtl">
              <div class="col footer_col">
                ...
                <span class="footer_label" style="margin-top: 11px;"> {{use_status}} </span>
              </div>

              <div class="col footer_col">
                <img alt="EVARA"  :src="require(`@/assets/images/evara_icon/ico_bath.jpg`)" style="height: 20px;width: auto;" class="footer_icon">
                <span class="footer_label" style="margin-top: 11px;"> {{bathroom}} {{$t('house_card.bathroom')}}</span>
              </div>

              <div class="col footer_col">
                <img alt="EVARA"  :src="require(`@/assets/images/evara_icon/ico_history.jpg`)" style="height: 20px;width: auto;" class="footer_icon">
                <span class="footer_label" style="margin-top: 11px;"> {{age_biulding}} {{$t('house_card.age_biulding')}}</span>
              </div>
            </div>
...
export default {
  ...
  , data() {
    return {
      ...
      share_title : '',
      share_desc : '',
      share_url : 'http://www.evara.life/en/evara_detail?estate='+this.id,
    }
  }
  , methods:{
    gotopage() {
      ...
      let routeData = this.$router.resolve(this.localePath({ path: '/detail/'+this.id + '.' + estate_title }));
    ...
    gotopage_3d() {
      ...
      var estate_title = this.urlHelper.replace('+', '-');
      let routeData = this.$router.resolve(this.localePath({ path: '/detail/'+this.id + '.' + estate_title, query: {show_3d_view: 1 } }));
      window.open(routeData.href, '_blank');
    },
    gotopage_bird_view() {
      this.$ga.event({
      ...
      var estate_title = this.urlHelper.replace('+', '-');
      let routeData = this.$router.resolve(this.localePath({ path: '/detail/'+this.id + '.' + estate_title, query: {show_birds_view: 1 } }));
      window.open(routeData.href, '_blank');
    },
    like_estate() {
      // if (this.$session.exists()) {
        this.$axios.$post('estate_like', {
          estate_id: this.id,
          ...
          .then((response) => {
            var rt_data = response[0];
            var res_data_res = rt_data.res;
            var res_data_msg = rt_data.message;
            if (res_data_res === 1) {
      ...
      //   this.$notify({
      //     type: 'warn',
      //     text: ' جهت پسندیدن باید عضو باشید '
      //   });
      // }
    },
    unlike_estate() {
      ...
      //   this.$notify({
      //     type: 'warn',
      //     text: ' جهت خارج کردن از حالت پسندیده باید عضو باشید '
      //   });
      // }
    },
    copyTestingCode () {
      let testingCodeToCopy = document.querySelector('#testing-code_'+this.id)
  ...
  , computed: {
    small_address: function () {

      var txt = '';
      var z = 0;
      for (var i = 0 ; i < this.address.toString().length;i++){
        txt += this.address[i];
        if (i === 35){
          txt += ' ... ';
          break;
  ...
  , mounted: function () {
    this.card_is_like = this.is_like;
    this.share_title = this.title;
    this.share_desc = this.address;
  }
}
</script>
