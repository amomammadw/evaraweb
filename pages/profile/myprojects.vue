    ...
    <div class="col" style="padding-top: 25px;">
      ...
      <div class="row" style="margin-bottom: 15px">
        <div class="col text-right" v-bind:class="[{'Estedad_FD_Bold' : $i18n.locale=='fa' || $i18n.locale=='ar' },{'Estedad_Bold' : $i18n.locale=='en' || $i18n.locale=='ru' || $i18n.locale=='tr'}]">
          <span>{{ $t('profile.your_registered_projects') }}</span>
        </div>
      </div>
        ...
        <div class="row">
          <div class="col">
            <table v-if="lst_estate.length > 0" class="table table-striped">
              ...
              <tr>
                <!--                      <th scope="col">#</th>-->
                <th scope="col">{{ $t('profile.projects_code') }}</th>
                <th scope="col">{{ $t('profile.projects_title') }}</th>
              ...
              <tr v-for="(item,indx) in lst_estate" :key="item.id" v-if="(item.app_action !== 0) || ((item.app_action === 0) && ((item.action_customer === 2) || (item.action_customer === 1)))" >
                ...
                <td class="table_text_td text-center" style="direction: ltr" v-if="item.app_action === 1"> <span class="badge badge-success">{{item.dealing_type}} {{ $t('profile.in_list') }}</span></td>
                <td class="table_text_td text-center" v-if="item.app_action === 0"> <span class="badge badge-warning"> {{ $t('profile.awaiting') }} </span> </td>
                <td class="table_text_td text-center" v-if="item.app_action === 2"> <span class="badge badge-danger">{{ $t('profile.suspend') }}</span> </td>
                <td class="table_text_td text-center" v-if="item.app_action === 3"><span class="badge badge-danger">{{ $t('profile.suspend') }}</span></td>
...
export default {
  ...
  , methods:{
    ...
    , fetch_estate () {
      ...
      this.$axios.$post('list_of_project_estate',{
      ...
      this.$axios.$post('update_action_customer',{
        ...
        .then((response) => {
          ...
          if (response[0].res === 1) {
            ...
            this.$ga.event({
              eventCategory: 'changeStatusProject',
              eventAction: 'click',
