    ...
    <div class="col" style="padding-top: 25px;">
      ...
      <div class="row" style="margin-bottom: 15px">
        <div class="col text-right" v-bind:class="[{'Estedad_FD_Bold' : $i18n.locale=='fa' || $i18n.locale=='ar' },{'Estedad_Bold' : $i18n.locale=='en' || $i18n.locale=='ru' || $i18n.locale=='tr'}]">
          <span>{{ $t('profile.done_deals') }}</span>
        </div>
      </div>
      ...
      <div class="row" style="margin-bottom: 15px">
        <div class="col text-right" v-bind:class="[{'Estedad_FD_Light' : $i18n.locale=='fa' || $i18n.locale=='ar' },{'Estedad_Light' : $i18n.locale=='en' || $i18n.locale=='ru' || $i18n.locale=='tr'}]">
          <small style="font-size: 10px;">{{ $t('profile.done_deals_desc') }}</small>
        </div>
      </div>
          ...
          <div class="col">
            <table v-if="lst_estate.length > 0" class="table table-striped">
              ...
              <tr>
                ...
                <th class="text-center" scope="col">{{ $t('profile.done_deals') }}</th>
                <th class="text-center" scope="col">{{ $t('profile.land_owner') }}</th>
                <th class="text-center" scope="col">{{ $t('profile.deal_price') }}</th>
                <th class="text-center" scope="col">{{ $t('profile.meeting_dt') }}</th>
              </tr>
              </thead>
              <tbody>
              <tr v-for="(item,indx) in lst_estate" v-if="item.deals !== 0">
                <th scope="row text-center">{{indx+1}}</th>
                ...
                <td class="table_text_td text-center">{{item.owner_full_name}}</td>
                <td class="table_text_td text-center" v-bind:class="[{'Estedad_FD_Bold' : $i18n.locale=='fa' || $i18n.locale=='ar' },{'Estedad_Bold' : $i18n.locale=='en' || $i18n.locale=='ru' || $i18n.locale=='tr'}]">-</td>
        ...
        .then((response) => {
          ...
          if (response[0].res === 1){
            for (var i = 0 ; i < response.length ; i++ ){
              if (response[i].deals !== 0){
                this.lst_estate.push(response[i]);
              }
            }

            this.pages_counts = parseInt(parseInt(lst_estate[0].count) / parseInt(this.viewer_count));
